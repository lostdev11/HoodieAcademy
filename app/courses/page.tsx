...
const techPathCoursesData: CourseCardProps[] = [
    ...
    },
    {
        ...
        description: "Master on-chain identity with .sol domains, learn domain management, and explore advanced SNS features.",
        badge: "Domain Master",
        emoji: "🌐",
        pathType: "tech",
        href: "/sns",
...
export default function CoursesPage() {
  ...
  useEffect(() => {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (savedStatus) {
          try {
            ...
            completedLessons = parsedStatus.filter(status => status === 'completed').length;
          } catch (e) {
            console.error("Failed to parse course progress from localStorage for key:", key, e);
          }
        }

        const isCompleted = totalLessonsForCourse > 0 && completedLessons === totalLessonsForCourse;
        const completionPercentage = totalLessonsForCourse > 0 ? (completedLessons / totalLessonsForCourse) * 100 : 0;
    ...
    ALL_COURSE_KEYS.forEach(key => {
      const { isCompleted, completionPercentage } = checkCompletion(key);
      status[key] = isCompleted;
      percentage[key] = completionPercentage;
    });

    setCourseCompletionStatus(status);
    // Check completion for ALL courses for Great Hoodie Hall
    setAllRequiredCoursesCompleted(ALL_COURSE_KEYS.every(key => status[key])); // Updated to check all keys
    setIsLoading(false);
  }, []);
  ...
  const isSnsCompleted = courseCompletionStatus[COURSE_LOCAL_STORAGE_KEYS.SNS];


  const socialPathCourses = socialPathCoursesData.map(course => {
    let isLocked = false;
    if (course.id === 'meme-coin-mania') {
      isLocked = false; // Meme Coin Mania is the first in Social Path
    } else if (course.id === 'crypto-x-influence') {
      // Crypto X Influence unlocks after Meme Coin Mania
      isLocked = !courseCompletionStatus[socialPathCoursesData.find(c => c.id === 'meme-coin-mania')!.localStorageKey!]; 
    } else if (course.id === 'community-strategy-social') {
      // Community Strategy (Social) unlocks after Crypto X Influence
      isLocked = !courseCompletionStatus[socialPathCoursesData.find(c => c.id === 'crypto-x-influence')!.localStorageKey!];
    }
    const courseKey = course.localStorageKey!;
    const completionPercentage = courseCompletionStatus[courseKey] ? 100 : 
      (typeof window !== 'undefined' && localStorage.getItem(courseKey) ? 
        (JSON.parse(localStorage.getItem(courseKey)!).filter(status => status === 'completed').length / course.totalLessons) * 100 : 0);

    return { 
      ...course, 
      isLocked: !isAdminBypass && isLocked, 
      isCompleted: courseCompletionStatus[courseKey],
  ...
  const techPathWalletWizardry = {
      ...
      isLocked: !isAdminBypass && false, // Wallet Wizardry is the first in Tech Path
      isCompleted: courseCompletionStatus[techPathCoursesData[0].localStorageKey!],
      completionPercentage: courseCompletionStatus[techPathCoursesData[0].localStorageKey!] ? 100 :
        (typeof window !== 'undefined' && localStorage.getItem(techPathCoursesData[0].localStorageKey!) ?
          (JSON.parse(localStorage.getItem(techPathCoursesData[0].localStorageKey!)!).filter(status => status === 'completed').length / techPathCoursesData[0].totalLessons) * 100 : 0),
  };
  const techPathNftMastery = {
    ...
    isCompleted: courseCompletionStatus[techPathCoursesData[1].localStorageKey!],
     completionPercentage: courseCompletionStatus[techPathCoursesData[1].localStorageKey!] ? 100 :
      (typeof window !== 'undefined' && localStorage.getItem(techPathCoursesData[1].localStorageKey!) ?
        (JSON.parse(localStorage.getItem(techPathCoursesData[1].localStorageKey!)!).filter(status => status === 'completed').length / techPathCoursesData[1].totalLessons) * 100 : 0),
  };
  const techPathCommunityStrategy = {
    ...techPathCoursesData[2],
    isLocked: !isAdminBypass && !isNftMasteryCompleted, // Community Strategy (Tech) unlocks after NFT Mastery
    isCompleted: courseCompletionStatus[techPathCoursesData[2].localStorageKey!],
     completionPercentage: courseCompletionStatus[techPathCoursesData[2].localStorageKey!] ? 100 :
      (typeof window !== 'undefined' && localStorage.getItem(techPathCoursesData[2].localStorageKey!) ?
        (JSON.parse(localStorage.getItem(techPathCoursesData[2].localStorageKey!)!).filter(status => status === 'completed').length / techPathCoursesData[2].totalLessons) * 100 : 0),
  };
  const techPathSns = {
    ...techPathCoursesData.find(c => c.id === 'sns')!,
    isLocked: !isAdminBypass && !isNftMasteryCompleted,
    isCompleted: isSnsCompleted,
    completionPercentage: isSnsCompleted ? 100 :
      (typeof window !== 'undefined' && localStorage.getItem(COURSE_LOCAL_STORAGE_KEYS.SNS) ?
        (JSON.parse(localStorage.getItem(COURSE_LOCAL_STORAGE_KEYS.SNS)!).filter(status => status === 'completed').length / techPathCoursesData.find(c => c.id === 'sns')!.totalLessons) * 100 : 0),
  };

  const tradingPathTechnicalAnalysis = {
    ...tradingPathCoursesData[0],
    isLocked: !isAdminBypass && false, // Technical Analysis is an independent starting path
    isCompleted: courseCompletionStatus[tradingPathCoursesData[0].localStorageKey!],
     completionPercentage: courseCompletionStatus[tradingPathCoursesData[0].localStorageKey!] ? 100 :
      (typeof window !== 'undefined' && localStorage.getItem(tradingPathCoursesData[0].localStorageKey!) ?
        (JSON.parse(localStorage.getItem(tradingPathCoursesData[0].localStorageKey!)!).filter(status => status === 'completed').length / tradingPathCoursesData[0].totalLessons) * 100 : 0),
  };

  const isGreatHoodieHallUnlocked = isAdminBypass || ALL_COURSE_KEYS.every(key => courseCompletionStatus[key]);
  ...
  return (
    <div className="flex flex-col items-center min-h-screen py-8 px-4 bg-background text-foreground">
      ...
      <main className="w-full max-w-4xl flex flex-col items-center py-8 space-y-10">
        ...
        {/* Add SNS Course Card in Stage 2 */}
         <div className="flex flex-col items-center space-y-4 w-full max-w-md mx-auto">
            <CourseCard {...techPathSns} pathType="tech"/>
         </div>
