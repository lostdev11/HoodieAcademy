...
export default function BountyManager({ bounties, onBountiesChange, walletAddress }: BountyManagerProps) {
  ...
  useEffect(() => {
    if (editingBounty) {
      setFormData({
        ...editingBounty,
        start_date: editingBounty.start_date ? editingBounty.start_date.split('T')[0] : '',
        deadline: editingBounty.deadline ? editingBounty.deadline.split('T')[0] : ''
      });
    } else if (showForm) {
      setFormData({
        title: '',
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <Card className="bg-slate-800">
          ...
          <CardContent>
            <form key={editingBounty?.id || 'new'} onSubmit={handleSubmit} className="space-y-4">
