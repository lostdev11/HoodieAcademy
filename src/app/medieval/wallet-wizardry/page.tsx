...
export default function WalletWizardryPage() {
  const [tierStatus, setTierStatus] = useState<
    Array<"locked" | "unlocked" | "completed">
  ...
  const [courseWalletAlertConfig, setCourseWalletAlertConfig] = useState({
    title: "",
    description: "",
  });
  const [courseConnectedWalletProvider, setCourseConnectedWalletProvider] =
    useState<WalletProviderOption | null>(null);
  const [showCourseWalletSelector, setShowCourseWalletSelector] =
    useState(false);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (savedStatus) {
        const parsedStatus: Array<"locked" | "unlocked" | "completed"> =
  ...
  const progressPercentage =
    (tierStatus.filter((s) => s === "completed").length / tiers.length) * 100;
  ...
  const handleCourseWalletConnection = async (
    providerName: WalletProviderOption
  ) => {
    ...
    try {
      switch (providerName) {
        case "metamask":
          ...
          if (window.ethereum.providers?.length) {
            ...
            if (!metaMaskProvider) {
              setCourseWalletAlertConfig({
                ...
                description:
                  "Please select MetaMask. If installed, ensure it's your active wallet or try refreshing.",
              });
              setShowCourseWalletAlert(true);
              return;
            }
            ...
            setCourseWalletAlertConfig({
              title: "MetaMask Required",
              description:
                "Please use MetaMask. Other Ethereum wallets are not supported for this action.",
            });
            setShowCourseWalletAlert(true);
            return;
          }
          ...
          setCourseMockNftStatus(
            Math.random() > 0.5
              ? "Mock Hoodie-Verse NFT: Owned!"
              : "Mock Hoodie-Verse NFT: None found."
          );
          setCourseWalletAlertConfig({
            title: "MetaMask Connected",
            description: `Successfully connected: ${userAccount.slice(
              0,
              6
            )}...${userAccount.slice(-4)}`,
          });
        ...
        case "magic-eden":
          ...
          if (providerName === "phantom") {
            ...
            if (window.solana && window.solana.isPhantom) solProvider = window.solana;
              ...
              setCourseWalletAlertConfig({
                ...
                description: `Please install a compatible Solana wallet (e.g., Phantom) for ${providerName}.`,
              });
              setShowCourseWalletAlert(true);
              return;
            }
          ...
          const solBalanceLamports = await solanaConnection.getBalance(
            new PublicKey(solAccount)
          );
          setCourseBalance(
            `${(solBalanceLamports / LAMPORTS_PER_SOL).toFixed(4)} SOL`
          );
          setCourseMockNftStatus(
            Math.random() > 0.5
              ? "Mock Solana NFT: Owned!"
              : "Mock Solana NFT: None found."
          );

          let successTitle =
            providerName.charAt(0).toUpperCase() + providerName.slice(1) + " Connected";
          if (providerName === "jup")
            successTitle = `Connected via ${solProvider.isPhantom ? "Phantom" : "Solana Wallet"} for JUP`;
          if (providerName === "magic-eden")
            successTitle = `Connected via ${solProvider.isPhantom ? "Phantom" : "Solana Wallet"} for Magic Eden`;

          setCourseWalletAlertConfig({
            title: successTitle,
            description: `Successfully connected: ${solAccount.slice(
              0,
              4
            )}...${solAccount.slice(-4)}`,
          });
          ...
          if (providerName === "jup") {
            setTimeout(
              () => alert("Simulating Jupiter Swap interaction: You can now trade tokens! (Mock)"),
              500
            );
          }
          if (providerName === "magic-eden") {
            setTimeout(
              () => alert("Simulating Magic Eden interaction: You can now browse NFTs! (Mock)"),
              500
            );
          }
          break;
        default:
          setCourseWalletAlertConfig({
            ...
            description: "This wallet provider is not yet supported.",
          });
      }
    } catch (error: any) {
      ...
      if (error.code === 4001 || error.message?.includes("User rejected the request")) {
        ...
        } not found. Please ensure it's installed and enabled.`;
      }
  ...
  const courseWalletProviders: {
    name: WalletProviderOption;
    label: string;
    icon?: JSX.Element;
  }[] = [
    {
    ...
    <div className="min-h-screen parchment-bg relative overflow-hidden">
      <div className="relative z-10 p-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold mb-4">Wallet Wizardry</h1>
          <p className="text-xl opacity-80 mb-2">
            Master wallet setup, security, and scam awareness.
          </p>
          <p className="text-lg">
            Current Time:{" "}
        ...
        <div className="max-w-3xl mx-auto">
          <TokenGate>
            <div className="flex flex-col items-center min-h-screen py-8 px-4 bg-background/40 text-foreground">
              <div className="w-full max-w-4xl mb-8 relative">
                ...
                <header className="text-center pt-16 md:pt-8">
                  ...
                  <p className="text-lg md:text-xl opacity-80">
                    Master wallet setup and security protocols to protect your Web3 assets,
                    earning the &apos;Vault Keeper&apos; NFT badge.
                  </p>
                  {/* Syllabus Button */}
                  <div className="mt-4">
                    <Button
              ...
              {showSyllabus && syllabusData["wallet-wizardry"] && (
                <div className="w-full max-w-4xl mb-8">
                  <Syllabus
                    data={syllabusData["wallet-wizardry"]}
                    courseTitle="Wallet Wizardry"
                  />
                </div>
              )}
              ...
              <main className="w-full max-w-3xl flex flex-col items-center py-8 space-y-10">
                <section className="my-8 w-full max-w-2xl text-center mx-auto">
                  ...
                  <p className="text-md">
                    The Wallet Wall&apos;s locks, powered by the &apos;First Thread,&apos; secure
                    the Hoodie-Verse&apos;s future.
                  </p>
                </section>

                <Progress
                  value={progressPercentage}
                  className="w-full max-w-3xl mb-8 bg-[hsl(var(--muted))]"
                />
                    ...
                    >
                        ...
                        <div className="ml-4">
                          {tierStatus[index] === "completed" ? (
                            ...
                            <div className="w-8 h-8 rounded-full flex items-center justify-center bg-[hsl(var(--accent))]">
                              <span className="text-[hsl(var(--accent-foreground))] text-sm font-bold">
                                {index + 1}
                              </span>
                            </div>
                          ) : (
                            <LockKeyhole className="w-8 h-8 opacity-60" />
                          )}
                        </div>
                      </div>
                      ...
                      {tierStatus[index] === "completed" ? (
                        <div className="text-center">
                          <p className="font-semibold mb-3">✓ Completed</p>
                          <Button
                            asChild
                            variant="outline"
                            size="sm"
                        ...
                        <Button
                          asChild
                          className="w-full shadow-scroll hover:shadow-glow"
                        >
                        ...
                        <div className="text-center">
                          <p className="opacity-70 text-sm">
                            Complete previous tiers to unlock
                          </p>
                        </div>
                      )}
                    </motion.div>
                ...
                {allTiersCompleted && (
                  ...
                  >
                    ...
                    <p className="text-lg mb-6">
                      You&apos;ve completed all tiers! Test your knowledge with the
                      comprehensive final exam.
                    </p>
                    <Button asChild className="shadow-scroll hover:shadow-glow">
                ...
                {!allTiersCompleted && (
                  ...
                  >
                    ...
                    <h2 className="text-3xl md:text-4xl font-bold mb-3">
                      Welcome to Wallet Wizardry!
                    </h2>
                    <p className="text-xl mb-4">
                      Complete all tiers to unlock the final exam and earn your{" "}
                      <span className="font-semibold">&apos;Vault Keeper&apos; NFT badge!</span>
                    </p>
                    <div className="flex items-center justify-center space-x-3 my-6">
                      <LockKeyhole className="w-12 h-12" />
                      <div>
                        <p className="text-lg md:text-xl font-semibold">
                          REWARD: Vault Keeper NFT
                        </p>
                        <p className="text-md opacity-80">
                          Master wallet security to earn your certification!
                        </p>
                      </div>
                    </div>
                ...
                <AlertDialog
                  open={showCourseWalletAlert}
                  onOpenChange={setShowCourseWalletAlert}
                >
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>
                        {courseWalletAlertConfig.title}
                      </AlertDialogTitle>
                      <AlertDialogDescription>
                    ...
                    <AlertDialogFooter>
                      {courseWalletAlertConfig.title !== "MetaMask Connected" &&
                        courseWalletAlertConfig.title !== "Phantom Connected" &&
                        !courseWalletAlertConfig.title.includes("Connected via") && (
                          <AlertDialogCancel>Cancel</AlertDialogCancel>
                        )}
                      <AlertDialogAction
                        onClick={() => setShowCourseWalletAlert(false)}
                        className="shadow-scroll hover:shadow-glow"
                      >
                        OK
                      </AlertDialogAction>
                    </AlertDialogFooter>
                ...
                <section className="my-8 text-center p-6 bg-card/90 rounded-2xl shadow-scroll hover:shadow-glow w-full animate-parchmentGlow">
                  <div className="flex justify-center items-center">
                    <Button
                      ...
                      className="px-6 py-3 mb-4 rounded-lg shadow-scroll hover:shadow-glow text-foreground min-w-[240px] transition-all duration-300 flex items-center justify-center"
                      aria-expanded={showCourseWalletSelector}
                    >
                      <Wallet size={18} className="mr-2" />
                      {courseConnectedWalletProvider
                        ? `Connected: ${courseConnectedWalletProvider}`
                        : "Select Wallet"}
                    </Button>
                  </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-md">
                        {courseWalletProviders.map((wallet) => (
                          <Button
                            ...
                            className="w-full px-6 py-3 rounded-lg shadow-scroll hover:shadow-glow transition-all duration-300 flex items-center justify-center"
                          >
                  ...
                  {courseAccount && (
                    ...
                    >
                      <p>
                        <strong>Address:</strong> {courseAccount}
                      </p>
                      <p>
                        <strong>Balance:</strong> {courseBalance || "Loading..."}
                      </p>
              ...
              <footer className="mt-12 text-center">
                <p className="text-sm opacity-70">#StayBuilding #StayHODLing</p>
              </footer>
            </div>
          </TokenGate>
        </div>
        ...
        {/* Footer hashtags */}
        <div className="mt-12 opacity-70 text-sm text-center">
