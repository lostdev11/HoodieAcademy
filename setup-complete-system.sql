-- Complete Hoodie Academy System Setup
-- This script sets up all required tables and dependencies in the correct order

-- ============================================================================
-- 1. SQUAD SELECTION SYSTEM (Required for course system)
-- ============================================================================
...
-- Wait a moment for the columns to be fully created, then create indexes
-- Create index for better query performance (only if columns exist)
DO $$
BEGIN
  -- Check if squad column exists before creating index
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'squad') THEN
    CREATE INDEX IF NOT EXISTS idx_users_squad ON users(squad);
  END IF;
  
  -- Check if squad_id column exists before creating index
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'squad_id') THEN
    CREATE INDEX IF NOT EXISTS idx_users_squad_id ON users(squad_id);
  END IF;
  
  -- Check if squad_lock_end_date column exists before creating index
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'squad_lock_end_date') THEN
...
-- ============================================================================
-- 2. COURSE SYSTEM (Depends on squad system)
-- ============================================================================
...
COMMENT ON TABLE courses IS 'Courses available in the academy, organized by squad';
COMMENT ON TABLE course_progress IS 'Tracks user progress through courses';
COMMENT ON TABLE course_sections IS 'Individual sections/modules within courses';
COMMENT ON TABLE section_progress IS 'Tracks user progress through individual course sections';

COMMENT ON COLUMN courses.squad_id IS 'Which squad this course belongs to (creators, decoders, speakers, traders, or all)';
COMMENT ON COLUMN courses.is_hidden IS 'Admin can hide courses from users';
COMMENT ON COLUMN course_progress.progress_percentage IS 'Percentage completion (0-100)';
COMMENT ON COLUMN course_progress.time_spent IS 'Total time spent on course in minutes';

-- Success message
DO $$
BEGIN
  RAISE NOTICE '✅ Complete Hoodie Academy system setup completed successfully!';
  RAISE NOTICE '📚 Course system with squad-based access control is ready';
  RAISE NOTICE '👥 Squad selection system is ready';
